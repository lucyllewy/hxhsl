...
if(!org.grumpytoad.hslexample.button) org.grumpytoad.hslexample.button = {}
org.grumpytoad.hslexample.button.Main = function(p) { if( p === $_ ) return; {
	var button = js.Lib.document.getElementById("button");
	var window = js.Lib.window;
	{
		var _g = 0;
		while(_g < 3) {
			var id = _g++;
			var button1 = js.Lib.document.getElementById("button" + (id + 1));
			var signaler = new org.hsl.js.translation.JSTranslatingSignaler(button1,button1,org.hsl.js.translation.JSEventType.MOUSEWHEEL,new org.hsl.js.translation.mouse.DeltaTranslator());
			signaler.addSlot($closure(this,"modifyColor"));
		}
	}
}}
org.grumpytoad.hslexample.button.Main.__name__ = ["org","grumpytoad","hslexample","button","Main"];
org.grumpytoad.hslexample.button.Main.main = function() {
	new org.grumpytoad.hslexample.button.Main();
}
org.grumpytoad.hslexample.button.Main.log = function(v,pos) {
	var console = Reflect.field(js.Lib.window,"console");
	if(console != null) console.log((((((pos.fileName + ":") + pos.className) + ":") + pos.methodName) + ":") + pos.lineNumber,v);
}
org.grumpytoad.hslexample.button.Main.prototype.combineColors = function() {
	var r = 0;
	var g = 0;
	var b = 0;
	{
		...
		while(_g < 3) {
			var id = _g++;
			var button = js.Lib.document.getElementById("button" + (id + 1));
			var color = button.style.backgroundColor;
			var yield = function(pos,v) {
				switch(pos) {
				case 0:{
					r += v;
				}break;
			...
			this.loopColors(color,yield);
		}
	}
	return ((((("rgb(" + r) + ", ") + g) + ", ") + b) + ")";
}
org.grumpytoad.hslexample.button.Main.prototype.loopColors = function(str,cb) {
...
org.grumpytoad.hslexample.button.Main.prototype.modifyColor = function(signal) {
	var button = signal.initialSubject;
	var data = signal.data;
	var color = button.style.backgroundColor;
	var combine = js.Lib.document.getElementById("combine");
	var yield = function(pos,v) {
		if(v != 0) v = ((data > 0)?((v < 254)?v + data:v):((v + data > 1)?v + data:v));
		return v;
	}
	button.style.backgroundColor = this.loopColors(color,yield);
	combine.style.backgroundColor = this.combineColors();
}
org.grumpytoad.hslexample.button.Main.prototype.__class__ = org.grumpytoad.hslexample.button.Main;
...
org.hsl.js.translation.mouse.DeltaTranslator = function(p) { if( p === $_ ) return; {
	null;
}}
org.hsl.js.translation.mouse.DeltaTranslator.__name__ = ["org","hsl","js","translation","mouse","DeltaTranslator"];
org.hsl.js.translation.mouse.DeltaTranslator.__super__ = org.hsl.js.translation.JSCommonTranslator;
for(var k in org.hsl.js.translation.JSCommonTranslator.prototype ) org.hsl.js.translation.mouse.DeltaTranslator.prototype[k] = org.hsl.js.translation.JSCommonTranslator.prototype[k];
org.hsl.js.translation.mouse.DeltaTranslator.prototype.translate = function(nativeEvent) {
	var mouseEvent = nativeEvent;
	var delta = 0;
	if(mouseEvent == null) mouseEvent = window.event;
	if(mouseEvent.wheelDelta) {
		if(js.Lib.isOpera) delta = mouseEvent.wheelDelta / 40;
		else delta = mouseEvent.wheelDelta / 120;
	}
	else if(mouseEvent.detail) {
		delta = -mouseEvent.detail;
	}
	var target = this.targetFromDOMEvent(mouseEvent);
	return new org.hsl.haxe.translation.Translation(((delta > 0)?Math.ceil(delta):Math.floor(delta)),target);
}
org.hsl.js.translation.mouse.DeltaTranslator.prototype.__class__ = org.hsl.js.translation.mouse.DeltaTranslator;
org.hsl.js.translation.mouse.DeltaTranslator.__interfaces__ = [org.hsl.haxe.translation.Translator];
...
js.Lib.onerror = null;
$Main.init = org.grumpytoad.hslexample.button.Main.main();
